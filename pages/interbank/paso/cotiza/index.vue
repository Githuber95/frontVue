<template lang="pug">
  section#cotiza
            ...
            div.data
              b-row.pb-2.pt-lg-3(v-if="!dataVacia") <!--Datos en resumen o los semicompletados -->
                ...
                b-col.text-right(cols="4")
                  b-button.py-1.w-auto.h-auto.editar(v-if="$store.state.common.channel === 'WEB'" variant="primary" @click="showSave" style="font-size: 14px;") Editar
                  b-button.py-1.w-auto.h-auto.editar(v-else variant="dark" @click="showSave" style="font-size: 14px;") Editar
    ...
    section
      ...
      b-modal#quote.abandono(static size="md" centered hide-footer hide-header :class="[(($store.state.common.subchannel).split('_VALE_')[0] === 'IBK_PROMOCION' || $store.state.common.subchannel === 'IBK_CROSS') && parseInt(formQuote.porcentaje) >= 100 ? 'bg-ibk' : '']")
        b-container(v-if="(($store.state.common.subchannel).split('_VALE_')[0] === 'IBK_PROMOCION' || $store.state.common.subchannel === 'IBK_CROSS') && parseInt(formQuote.porcentaje) >= 100")
          b-row.justify-content-center
            b-col.text-center.mb-5(cols="12")
              img(src="@/static/media/interbank/abandono_promocion.svg" alt="promocion de interbank")
          b-row.text-center
            b-col(cols="12")
              h2.omnes-semibold.mb-0.text-primary ¡Gana vales o cuota gratis !
            b-col(cols="12").my-4
              ...
              h3.text-primary Gana sin sorteos un vale de hasta<span class="omnes-semibold">S/{{($store.state.common.subchannel).split('_VALE_')[1]}}</span><br> en planes semestrales, trimestrales y anuales o la prímera cuota gratis en plan mensual con vale de S/80.
          b-row.justify-content-center
            b-col.text-center(cols="12")
