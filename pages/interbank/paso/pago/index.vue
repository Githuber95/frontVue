<template lang="pug">
  section#pago(ref="pago")
      ...
      b-row.box-resumen.justify-content-center.px-xl-5
        ...
        b-col.editar.d-lg-block.d-none.px-xl-5(cols="12" md="8" lg="6" xl="5")
          div.w-100.resumen.mb-5
            b-container
              b-row.title.my-2.my-xl-0.justify-content-center
                b-col(cols="12")
    ...
    b-modal#pay.abandono(static size="md" centered hide-footer hide-header :class="[(($store.state.common.subchannel).split('_VALE_')[0] === 'IBK_PROMOCION' || $store.state.common.subchannel === 'IBK_CROSS') && parseInt($store.state.common.cotizacion.porcentaje) >= 100 ? 'bg-ibk' : '']")
      b-container(v-if="(($store.state.common.subchannel).split('_VALE_')[0] === 'IBK_PROMOCION' || $store.state.common.subchannel === 'IBK_CROSS') && parseInt($store.state.common.cotizacion.porcentaje) >= 100")
        b-row.justify-content-center
