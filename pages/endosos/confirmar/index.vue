    ...
    section#confirm.pay(v-if="parseFloat($store.state.common.primaClienteAnual) <= maxPrimaAnual || stepClient === '3'")
      b-container
        b-row.justify-content-md-center
          b-col.text-center.mt-xl-4(md="7" lg="4" xl="5")
            h2.omnes-medium.text-center.mb-0.my-3.d-xl-inline ¡{{ (($store.state.common.saveClient.nombre).split(' ')[0]).toUpperCase() }}, tu compra ha sido <span class="d-block omnes-medium d-xl-inline"> realizada con éxito!</span>
        b-row.justify-content-md-center.mt-xl-5
          b-col.text-center(md="5" xl="4")
            h3.text-center.mb-3.d-block.omnes-medium Estos son los datos de tu póliza {{ $store.state.payment.numeroPoliza }}
        div.datos-confirmacion
          b-row.text-primary.justify-content-center
            b-col.mx-xl-5.px-1.px-xl-4(md="7" lg="5")
              // Inicio de vigencia
            ...
            div.w-100
            b-col.mx-xl-5.px-1.px-xl-4(md="7" lg="5").text-secondary
              ...
              b-col.bg-secondary-opacity(cols="12")
                b-row
                  ...
                  b-col.text-right.pl-0(cols="4" xl="5")
                    p.omnes-medium S/ {{ $store.state.common.cotizacion.sumaAsegurada.toString().split(/(?=(?:\d{3})+$)/).join(',') }}
              // Protección por fallecimiento
              b-col.bg-secondary-opacity(cols="12" v-if="Object.keys($store.state.common.cotizacionAdicional).length && $store.state.common.channel === 'ENDOSOS'")
                b-row
                  b-col.pr-xl-0(cols="8" xl="7")
                    p Protección por invalidez
                  b-col.text-right.pl-0(cols="4" xl="5")
                    p.omnes-medium S/ {{ parseDecimal($store.state.common.cotizacionAdicional[1].sumaAsegurada) }}
              // Pagarás
              b-col.bg-secondary-opacity(cols="12")
                b-row
                  b-col(cols="8" xl="7")
                  ...
                  b-col.text-right.pl-0(cols="4" xl="5")
                    p.omnes-medium S/ {{ (parseFloat($store.state.common.cotizacion.primaRecurrente) + parseFloat($store.state.common.cotizacionAdicional[1].primaRecurrente)).toFixed(2)}}
              // Devolución
              b-col.bg-secondary-opacity(cols="12")
                b-row
                  b-col(cols="8" xl="7")
              ...
              b-col.bg-secondary-opacity(cols="12")
                b-row
                  ...
                  b-col.text-right.pl-0(cols="4" xl="5")
                    p.omnes-medium S/ {{ parseDecimal(parseFloat($store.state.common.cotizacion.montoDevolucion) + parseFloat($store.state.common.cotizacionAdicional[1].montoDevolucion)) }}
        b-row.text-center.justify-content-center.mt-3.mt-lg-4.verificar-datos(v-if="$store.state.common.hashPhone || $store.state.common.hashEmail")
          b-col.text-center(md="12" xl="10")
            h3.text-center.mb-3.d-block.omnes-medium Por favor, verifica tus datos para <br> el envío de la póliza
          b-col(cols="12")
          b-col.px-1.px-xl-4.mx-xl-5.my-2(cols="12" md="7" lg="5")
        ...
        b-row.text-center.justify-content-center
          b-col.mx-xl-5.px-xl-4(md="7" lg="5" v-if="!$store.state.common.hashPhone && !$store.state.common.hashEmail")
            b-button.my-4.py-2(variant="dark" @click="goHome") Entendido
          b-col.mx-xl-5.px-xl-4(md="7" lg="5" v-else)
            b-button.my-4.py-2.mb-4(variant="dark" @click="updateSendPoliza") Enviar póliza
            h3.text-primary.omnes-medium(@click="goHome" style="cursor: pointer;") SALIR
        b-row.justify-content-center(v-if="!$store.state.common.hashPhone && !$store.state.common.hashEmail")
          b-col.text-center(md="6" lg="5")
            p(style="font-size: 18px") Hemos enviado el detalle de tu póliza a:
            p.omnes-semibold.text-primary(style="font-size: 16px") {{ $store.state.common.saveClient.email }}
            p.mt-2(style="font-size: 18px" v-if="$store.state.common.channel == 'ENDOSO'") ¡Comparte lo fácil que fue comprar tu Vida con devolución!
            p.mt-2(style="font-size: 18px" v-else) ¡Comparte lo fácil que fue comprar tu Seguro Vida Cash!
        b-row.justify-content-center.mt-3.mb-5
          b-col(cols="8" sm="6" md="4" lg="3" xl="3")
            b-row.justify-content-center
              b-col.pl-0(cols="3")
                div.shared
                  a( target="_blank" :href="`https://www.facebook.com/sharer/sharer.php?u=${urlShared}${utmFacebook}`" @click="socialEventFacebook()")
                    img.centerimg(src="@/static/media/icons/facebook_rspv.svg" alt="Seguro de vida, Interseguro en Facebook" style="width: 45px;")
              b-col.pl-0(cols="3")
                div.shared
                  a(target="_blank" :href="`https://web.whatsapp.com/send?text=${textoComparte} ${urlShared}${utmwhatsapp}`"  @click="socialEventWhatsapp()")
                    img#whatsappIConDesktop.centerimg(src="@/static/media/icons/whatsapp.svg" alt="Seguro de vida contacta al whatsapp" style="width: 45px;")
              b-col.pl-0(cols="3")
                div.shared
                  a(target="_blank" :href="`https://twitter.com/intent/tweet?text=${textoComparte}&url=${urlShared}${utmTwitter}`" @click="socialEventTwitter()")
                    img#twticonDesktop.centerimg(src="@/static/media/icons/twitter_rspv.svg" alt="Seguro de vida, Facebook en twitter" style="width: 45px;")
              b-col.pl-0(cols="3")
                div.shared
                  a(target="" href="" id="tooltipButton-Desktop" @click="copyURL($event, 'desktop')")
                    img.centerimg(src="@/static/media/icons/copy_rspv.svg" alt="Seguro de vida, Copiar URL" style="width: 45px;")
                div
                  b-popover(target="tooltipButton-Desktop" placement="top")
                    p.text-popOverSuccess.omnesMedium Link Copiado!
                    input(id="desktop_url")
    section#confirm.steps(v-else)
      ...
      b-container(v-if="stepClient === '1'")
        b-row.justify-content-center.mt-3
          b-col.text-center.borde(cols="11" md="5")
...
export default {
  ...
  mounted () {
    ...
    this.urlShared = window.location.origin + '/seguro-de-vida/interbank'
    ...
    if (this.$store.state.payment.numeroPoliza) {
      ...
      this.$nuxt.$router.push({ path: '/seguro-de-vida/interbank' })
    }
  },
  methods: {
    copyURL (e, type) {
      ...
      let self = this
      setTimeout(function () {
        this.urlShared = window.location.origin + '/seguro-de-vida'
  ...
  beforeDestroy () {
    ...
    if (localStorage.removeItem('jwt') !== null) {
      this.$store.commit('common/setSetCotizacion', [])
