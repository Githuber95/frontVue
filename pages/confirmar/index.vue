<template lang="pug">
  div(ref="pay")
    section#confirm.pay(v-if="parseFloat($store.state.common.primaClienteAnual) <= maxPrimaAnual || stepClient === '3'")
      b-container.pb-3.pb-lg-5
        b-row.justify-content-md-center
          b-col(cols="12")
            .pila.bg-primary.py-2
              h2.text-white.text-center ¡Felicitaciones! Te damos la bienvenida a la familia Interseguro 
        b-row.mt-2
          b-col(cols="12" lg="7")
            div.bg-white.all-border.py-lg-3.px-lg-2
              b-row.text-primary.justify-content-center.box-confirm
                b-col.d-flex(cols="12")
                  div.d-flex.w-100(style="background:#E2FFF2;")
                    .bg-border 
                    div.align-items-center.d-flex
                      img.px-2(src="@/static/media/common/pago/check_pago.svg" alt="confirmacion de compra")
                      h4(style="color: #00A888;") Tu compra se realizó con éxito
              b-row.text-primary.justify-content-center.mt-2
                b-col.text-center(cols="12")
                  h4.text-black.omnes-medium Ahora vive con tranquilidad 
                  h2.text-primary.omnes-medium ¡Gracias {{(($store.state.common.saveClient.nombre).split(' ')[0]).toUpperCase() }} por adquirir tu <br> {{ currentChannel }} !
                  //- h2.text-primary ¡Gracias {{ $store.state.common.saveClient.nombre }} por adquirir tu <br> {{ currentChannel }} !
              div.datos-confirmacion
                b-row.text-primary.justify-content-center
                  b-col.mt-2.text-center(cols="12")
                    h4.text-black.omnes-medium Estos son los datos de tu póliza N° {{ $store.state.payment.numeroPoliza }}
                  b-col.mx-xl-5(cols="12")
                    // Inicio de vigencia
                    b-col.bg-secondary-opacity(cols="12")
                      b-row.px-0
                        b-col(cols="8")
                          p Inicio de vigencia
                        b-col.text-right.pl-0(cols="4")
                          p.omnes-medium {{ today }}
                    // Periodo
                    b-col.bg-secondary-opacity.mb-0(cols="12")
                      b-row.px-0
                        b-col(cols="8")
                          p Periodo
                        b-col.text-right.pl-0(cols="4")
                          p.omnes-medium {{ $store.state.common.cotizacion.periodoPago }} años
                  div.w-100
                  b-col.mx-xl-5(cols="12").text-secondary
                    // Protección por fallecimiento
                    b-col.bg-secondary-opacity(cols="12")
                      b-row.px-0
                        b-col.pr-xl-0(cols="8" xl="7")
                          p Protección por fallecimiento
                        b-col.text-right.pl-0(cols="4" xl="5")
                          p.omnes-medium S/ {{ $store.state.common.cotizacion.sumaAsegurada }}
                    // Protección por fallecimiento
                    b-col.bg-secondary-opacity(cols="12" v-if="Object.keys($store.state.common.cotizacionAdicional).length")
                      b-row.px-0
                        b-col.pr-xl-0(cols="8" xl="7")
                          p {{ $store.state.common.cotizacionAdicional[1].description }}
                        b-col.text-right.pl-0(cols="4" xl="5")
                          p.omnes-medium S/ {{ $store.state.common.cotizacionAdicional[1].sumaAsegurada }}
                    // Pagarás
                    b-col.bg-secondary-opacity(cols="12")
                      b-row.px-0
                        b-col(cols="8" xl="7")
                          p(v-if="$store.state.common.cotizacion.frecuenciaPago === 'M'") Pagarás mensual
                          p(v-if="$store.state.common.cotizacion.frecuenciaPago === 'T'") Pagarás trimestral
                          p(v-if="$store.state.common.cotizacion.frecuenciaPago === 'S'") Pagarás semestral
                          p(v-if="$store.state.common.cotizacion.frecuenciaPago === 'A'") Pagarás anual
                        b-col.text-right.pl-0(cols="4" xl="5" v-if="Object.keys(this.$store.state.common.cotizacionAdicional).length")
                          p.omnes-medium S/ {{ parseDecimal(parseFloat(this.$store.state.common.cotizacion.primaRecurrente) + parseFloat(this.$store.state.common.cotizacionAdicional[1].primaRecurrente)) }}
                        b-col.text-right.pl-0(cols="4" xl="5" v-else)
                          p.omnes-medium S/ {{ parseDecimal($store.state.common.cotizacion.primaRecurrente) }}
                    // Tiempo
                    b-col.bg-secondary-opacity.mb-0(cols="12")
                      b-row.px-0
                        b-col(cols="8")
                        ...
                        b-col.text-right.pl-0(cols="4")
                          p.omnes-medium {{ $store.state.common.cotizacion.periodoPago }} años
                    // Devolución
                    b-col.bg-secondary-opacity(cols="12" v-if="$store.state.common.channel !== 'PROTECCION_SEGURA'")
                      b-row.px-0
                        b-col(cols="8" xl="7")
                          p Devolución
                        b-col.text-right.pl-0(cols="4" xl="5")
                          p.omnes-medium {{ $store.state.common.cotizacion.porcentaje }}%
                    // Monto final a devolver
                    b-col.bg-secondary-opacity(cols="12" v-if="$store.state.common.channel !== 'PROTECCION_SEGURA'")
                      b-row.px-0
                        b-col(cols="8" xl="7")
                          p Monto final a devolver
                        b-col.text-right.pl-0(cols="4" xl="5")
                          p.omnes-medium S/ {{ parseDecimal($store.state.common.cotizacion.montoDevolucion) }}          
              hr
              b-row.justify-content-center.mt-3.mt-lg-4.verificar-datos(v-if="$store.state.common.hashPhone || $store.state.common.hashEmail")
                b-col(cols="12")
                  h3.d-block.omnes-medium Por favor, verifica tus datos para el envío de la póliza
                  h6.omnes-light.mt-2.mb-3 Al momento de la compra hemos enviado automáticamente un correo con la confirmación de la compra de tu seguro al correo que figura en la parte superior.
                b-col(cols="12" lg="6")
                  b-form-input.py-0.h-auto#numeroTelefono(autocomplete="off" maxLength="9" type='tel' placeholder='Celular' v-model="client.numeroTelefono" @keydown.native="onlyNumber($event)" @keyup.native="onlyNumber($event)" @keypress.native="onlyNumber($event)" :state="$v.client.numeroTelefono.$dirty ? !$v.client.numeroTelefono.$error : null" aria-describedby="numeroTelefonoFeedback")
                ...
                b-col(cols="12" lg="6")
                  b-form-input.py-0.h-auto#email(autocomplete="off"  type='text' placeholder='Correo electrónico' v-model="client.correo" :state="$v.client.correo.$dirty ? !$v.client.correo.$error : null" aria-describedby="emailFeedback")
                ...
                b-col(cols="12"  v-if="$store.state.common.hashPhone && $store.state.common.hashEmail")
                  b-button.my-2.py-2(variant="dark" @click="updateSendPoliza") Enviar póliza
              b-row.justify-content-center(v-if="!$store.state.common.hashPhone && !$store.state.common.hashEmail")
                b-col.text-center(cols="12")
                  p(style="font-size: 18px") Hemos enviado el detalle de tu póliza a:
                  p.omnes-semibold.text-primary(style="font-size: 16px") {{ client.correo }}
            .mt-3.bg-white.all-border.py-lg-3.px-lg-2
              b-row  
                b-col
                  div.pila.bg-primary.py-1(style="width: fit-content")
                    h3.text-white.px-2 Zona Segura
              b-row.mt-2
                b-col.pr-lg-0(cols="8" lg="10")
                  h3.text-primary.omnes-medium El espacio de autogestión para clientes
                  h6.omnes-semibold(style="color: #787C94;") Revisa y controla tus pagos, cambia tu método de pago,
                    | conoces todos tus beneficios <span class="omnes-regular"> y mucho más 
                    | en la Zona Segura, un espacio exclusivo para nuestros clientes. Accede a tu espacio en el siguiente enlace </span>
                b-col.pl-0.d-none.d-lg-block(lg="2")
                  img.text-center.w-100(src="@/static/media/common/pago/autogestion_clientes.svg" alt="autogestion familia protegida")
                b-col.my-3(cols="12")
                  a(href="https://www.interseguro.pe/zona-segura/" target="_blank")
                   b-button.py-2.w-auto.h-auto(variant="dark" @click="goZonaSegura") Accede a Zona Segura
          b-col(cols="12" lg="5")
            div.bg-white.all-border.py-lg-3(v-if="parseInt($store.state.common.cotizacion.porcentaje) >= 100 && parseInt($store.state.common.cotizacion.sumaAsegurada) >= 100000")
              b-row  
                b-col(cols="12")
                  div.pila.bg-primary(style="width: fit-content")
                    h3.text-white.px-2.py-1 Accediste a la siguiente campaña
              b-row.mt-2
                b-col.mb-2(cols="12")
                  div.banner_campania.py-2.px-1
                    img.w-100(src="@/static/media/common/home/banner_descuento-responsive.svg")
              //- b-row.mt-2  
                b-col(cols="1")
                  img(src="@/static/media/common/pago/check_green.svg" alt="listo" style="width: 1.5rem;")
                b-col(cols="11")
                  h5.omnes-medium.text-primary Devolución de la primera cuota gratis
                  h5 Solo aplica en planes mensuales, la devolución se realiza en los 
                    | primeros 7 días hábiles, pero puede demorar hsata 30 días por reglas 
                    | del banco donde se realizó el abono
              b-row.mt-2  
                b-col(cols="1")
                  img(src="@/static/media/common/pago/check_green.svg" alt="listo" style="width: 1.5rem;")
                b-col(cols="11")
                  h5.omnes-medium.text-primary 10% dcto. en seguro de tu seguro de viajes
                  h5 Perfecto para planificar tu próximo viaje seguro, con la compra
                    | de tu seguro de vida, llévate esta promoción especial
              b-row.mt-2  
                b-col(cols="1")
                  img(src="@/static/media/common/pago/check_green.svg" alt="listo" style="width: 1.5rem;")
                b-col(cols="11")
                  h5.omnes-medium.text-primary Disfruta tu vale Agora  de S/250 
                  h5 Por la compra de tu seguro de vida, recibirás un vale de (el monto específico), 
                    |  cual lo recibirás entre (las fechas que enviarémos)  para que disfrutes en lo más quieres
            .pila.bg-primary.my-2.py-1(:class="[parseInt($store.state.common.cotizacion.porcentaje) >= 100 && parseInt($store.state.common.cotizacion.sumaAsegurada) >= 100000 ? '' : 'mt-3']")
              h4.text-white Los beneficios que tienes con tu <br>{{ currentChannel }}
            .bg-white.all-border.py-2
              b-row.mt-2.align-items-center 
                b-col.text-center(cols="2")
                  img.text-center(src="@/static/media/common/pago/pago_asistencia_medica.svg" alt="asistencia medica")
                b-col(cols="10")
                  h5.omnes-semibold.text-primary Asistencias médicas
                  h5.text-primary Envío de médico a domicilio (Medicina general) <span class="omnes-semibold">por S/35</span>
              b-row.mt-2.align-items-center 
                b-col.text-center(cols="2")
                  img.text-center(src="@/static/media/common/pago/pago_asistencia_hogar.svg" alt="asistencia medica")
                b-col(cols="10")
                  h5.omnes-semibold.text-primary Asistencia Hogar
                  h5.text-primary Envío de gasfitero, electricista, vidriero y cerrajero a domicilio 
                    | por emergencia con cobertura <span class="omnes-semibold">hasta S/300</span>
              b-row.mt-2.align-items-center 
                b-col.text-center(cols="2")
                  img.text-center(src="@/static/media/common/pago/pago_asistencia_vial.svg" alt="asistencia medica")
                b-col(cols="10")
                  h5.omnes-semibold.text-primary Asistencia Vial
                  h5.text-primary Traslado de vehículo en grúa por emergencia con cobertura <span class="omnes-semibold">hasta S/300</span>
              b-row.mt-2.align-items-center 
                b-col.text-center(cols="2")
                  img.text-center(src="@/static/media/common/pago/pago_asistencia_mascota.svg" alt="asistencia mascota")
                b-col(cols="10")
                  h5.omnes-semibold.text-primary Asistencia Mascota
                  h5.text-primary Servicio de baño por mascota <span class="omnes-semibold">por S/15</span>
              hr
              b-row.mt-2  
                b-col(cols="12")
                  h6(style="color: #787C94;") Accede a estos beneficios y a mucho más en la 
                    | <span class="omnes-semibold">Zona Segura</span> para clientes que tenemos para ti
    section#confirm.steps(v-else)
      b-container
        b-row.justify-content-center
          b-col.text-center.mt-3(md="5" lg="12")
            h3.text-center.mb-3.d-block.omnes-medium.text-primary Gracias por asegurar tu futuro <br class="d-lg-none"> y de los que m&aacute;s amas
        b-row.justify-content-center
          b-col.text-center(md="5" lg="12")
            h3.text-center.mb-3.d-block Para poder enviarte tu p&oacute;liza, necesitamos <br> la siguiente informaci&oacute;n.
      b-container(v-if="stepClient === '1'")
        b-row.justify-content-center.mt-3
          b-col.text-center.borde.bg-white.mb-3(cols="11" md="5")
            b-row.justify-content-center.mt-lg-5
              b-col.text-center(md="12")
                h3.text-center.mb-3.d-block.omnes-medium.text-primary ¿Cu&aacute;l es tu profesi&oacute;n?
              b-col.text-center(cols="12")
                b-form-select.browser-default.mb-2#depositare.ml-0(v-model="formPlaft.profesion" :options="profesion" value-field="codigo" text-field="nombre")
              b-col.text-center.mt-5(md="12")
                b-button.my-4.py-2(variant="dark" @click="goSteps('2')" :class="[formPlaft.profesion != '0' ? '' : 'disabled']") Continuar
      b-container(v-if="stepClient === '2'")
        b-row.justify-content-center.mt-xl-5.mb-lg-5
          b-col.text-center.borde.bg-white.mb-3(cols="11" md="5")
            b-row.justify-content-center.mt-lg-5
              b-col.text-center(md="12")
                h3.text-center.mb-3.d-block.omnes-medium.text-primary Ingresa la direcci&oacute;n de tu domicilio
              b-col.text-center(cols="12")
                b-form-select.browser-default.text-center.mb-2#depositare.ml-0(v-model="formPlaft.departamento" :options="departamento" @change="updateDep" value-field="id" text-field="name")
            b-row.justify-content-center
              b-col.text-center(cols="12")
                b-form-select.browser-default.text-center.mb-2#depositare.ml-0(v-model="formPlaft.provincia" :options="provinciaFilter" @change="updateProv" value-field="id" text-field="name")
            b-row.justify-content-center
              b-col.text-center(cols="12")
                b-form-select.browser-default.text-center.mb-2#depositare.ml-0(v-model="formPlaft.distrito" :options="distritoFilter" value-field="id" text-field="name")
            b-row.justify-content-center
              b-col.text-center(cols="12")
                b-form-input.browser-default.text-center.mb-2#depositare.ml-0(v-model="formPlaft.direccion" placeholder="Dirección")
            b-row.justify-content-center
              b-col.text-center(cols="12")
                b-button.my-4.py-2(variant="dark" @click="goSteps('3')" :class="[(!formPlaft.departamento || !formPlaft.provincia || !formPlaft.distrito || !formPlaft.direccion) ? 'disabled' : '']") Continuar
</template>
...
export default {
  ...
  data () {
    return {
      currentChannel: 'Vida con Devolución',
      maxPrimaAnual: 5400, 
      stepClient: '1',
      formPlaft: {
        profesion: '0',
      ...
      profesion: [
        { "codigo": "0", "nombre": 'Selecciona', disabled: true },
        {
          "codigo": "1",
          "nombre": "Abogado",
        },
        {
          "codigo": "2",
          "nombre": "Actor, actriz, director de espectáculos o artista",
        },
        {
          "codigo": "3",
          "nombre": "Actuario",
        ...
        {
          "codigo": "7",
          "nombre": "Agente/Corredor inmobiliario",
        },
        {
          "codigo": "8",
          "nombre": "Agente de bolsa",
        },
        {
          "codigo": "9",
          "nombre": "Agente de migración",
        },
        {
          "codigo": "10",
          "nombre": "Agente de turismo",
        },
        {
          "codigo": "11",
          "nombre": "Agente/Corredor de seguros",
        },
        {
          "codigo": "12",
          "nombre": "Agricultor, Agrónomo",
        },
        {
          "codigo": "13",
          "nombre": "Albañil, obrero de construcción",
        },
        {
          "codigo": "14",
          "nombre": "Ama de casa",
        ...
        {
          "codigo": "15",
          "nombre": "Analista de sistema y computación",
        },
        {
          "codigo": "16",
          "nombre": "Antropólogo, arqueólogo y etnólogo",
        },
        {
          "codigo": "17",
          "nombre": "Archivero",
        ...
        {
          "codigo": "22",
          "nombre": "Autor literario, escritor y critico",
        },
        {
          "codigo": "23",
          "nombre": "Avicultor",
        ...
        {
          "codigo": "24",
          "nombre": "Bacteriólogo, farmacólogo, biólogo, científico",
        },
        {
          "codigo": "25",
          "nombre": "Basurero/Barrendero",
        },
        {
          "codigo": "26",
          "nombre": "Cajero",
        ...
        {
          "codigo": "27",
          "nombre": "Camarero/barman/mesero/cocinero/chef",
        },
        {
          "codigo": "28",
          "nombre": "Cambista, compra/venta de moneda",
        },
        {
          "codigo": "29",
          "nombre": "Campesino",
        ...
        {
          "codigo": "36",
          "nombre": "Comerciante/Vendedor",
        },
        {
          "codigo": "37",
          "nombre": "Conductor, chofer/taxista",
        },
        {
          "codigo": "38",
          "nombre": "Conserje/Portero/Guardían/Vigilante",
        },
        {
          "codigo": "39",
          "nombre": "Constructor",
        ...
        {
          "codigo": "42",
          "nombre": "Corte y confección de ropa/Fabicante de prenda",
        },
        {
          "codigo": "43",
          "nombre": "Cosmetólogo, Peluquero y barbero",
        },
        {
          "codigo": "44",
          "nombre": "Decorador, dibujante, publicista",
        },
        {
          "codigo": "45",
          "nombre": "Dentista/Odontólogo",
        },
        {
          "codigo": "46",
          "nombre": "Deportista prefesional, atleta, arbitro",
        },
        {
          "codigo": "47",
          "nombre": "Distribuidor",
        ...
        {
          "codigo": "51",
          "nombre": "Empleada(o) del hogar / Nana",
        },
        {
          "codigo": "52",
          "nombre": "Empresario",
        ...
        {
          "codigo": "57",
          "nombre": "Fotógrafo/Operadores cámara",
        },
        {
          "codigo": "58",
          "nombre": "Ganadero",
        ...
        {
          "codigo": "65",
          "nombre": "Joyero y/o Platero",
        },
        {
          "codigo": "66",
          "nombre": "Jubilado/Pensionista",
        },
        {
          "codigo": "67",
          "nombre": "Laboratorista(Técnico)",
        },
        {
          "codigo": "68",
          "nombre": "Liquidador, reclamaciones/seguros",
        },
        {
          "codigo": "69",
          "nombre": "Maquinista/Operador de maquianria",
        },
        {
          "codigo": "70",
          "nombre": "Martillero/Subastador",
        },
        {
          "codigo": "71",
          "nombre": "Mayorista, comercio al por mayor",
        },
        {
          "codigo": "72",
          "nombre": "Mecánico",
        ...
        {
          "codigo": "75",
          "nombre": "Miembros de las fuerzas armadas",
        },
        {
          "codigo": "76",
        ...
        {
          "codigo": "89",
          "nombre": "Productor de Cine/Radio/Televisón",
        },
        {
          "codigo": "90",
          "nombre": "Productor, Cultivos extensivos",
        },
        {
          "codigo": "91",
          "nombre": "Programador",
        ...
        {
          "codigo": "92",
          "nombre": "Psicólogo/Terapeuta",
        },
        {
          "codigo": "93",
          "nombre": "Quiropráctico/Kinesiterapeuta",
        },
        {
          "codigo": "94",
          "nombre": "Relacionista publico e inductrial",
        },
        {
          "codigo": "95",
          "nombre": "Relojero",
        ...
        {
          "codigo": "96",
          "nombre": "Reparación de automóviles",
        },
        {
          "codigo": "97",
          "nombre": "Reparador de aparatos electrodomésticos",
        },
        {
          "codigo": "98",
          "nombre": "Repartidor",
        ...
        {
          "codigo": "99",
          "nombre": "Sacerdote/Monja",
        },
        {
          "codigo": "100",
          "nombre": "Secretaria, Recepcionista, Telefonista",
        },
        {
          "codigo": "101",
          "nombre": "Seguridad/Guardaespaldas",
        },
        {
          "codigo": "102",
          "nombre": "Servicio de almacenamiento/Almacenero",
        },
        {
          "codigo": "103",
          "nombre": "Servicio de alquiler de vehículos",
        },
        {
          "codigo": "104",
          "nombre": "Servicio de alquiler de videos, equipos de sonido",
        },
        {
          "codigo": "105",
          "nombre": "Sociólogo",
        ...
        {
          "codigo": "110",
          "nombre": "Transporte de carga y/o mudanza",
        },
        {
          "codigo": "111",
        ...
        {
          "codigo": "113",
          "nombre": "Veterinario, Zoólogo, Zootécnico",
        },
        {
          "codigo": "114",
          "nombre": "Visitador técnico",
        },
        {
          "codigo": "115",
          "nombre": "Zapatero",
      ...
      departamento: 
      [
        { "id": "0", "name": 'Departamento', disabled: true },
        {"id":"01","name":"Amazonas"},
        {"id":"02","name":"Ancash"},
        {"id":"03","name":"Apurimac"},
        {"id":"04","name":"Arequipa"},
      ...
      provincia: 
      [
        { "id": "0", "name": 'Provincia', disabled: true },
      ...
      provinciaFilter:  [{ "id": "0", "name": 'Provincia', disabled: true }],
      distritoFilter: [{ "id": "0", "name": 'Distrito', disabled: true }],
      distrito: 
      [
        { "id": "0", "name": 'Distrito', disabled: true },
      ...
      show_mobile: false,
      urlShared: '',
      utmFacebook: '/?utm_source=Facebook%26utm_medium=share%26utm_campaign=share',
      utmTwitter: '/?utm_source=Twitter%26utm_medium=share%26utm_campaign=share',
      utmwhatsapp: '/?utm_source%3DWhatsapp%26utm_medium%3Dshare%26utm_campaign%3Dshare',
      textoComparte: 'Ahora puedo proteger a quienes más amo y recuperar todo mi dinero con Vida con devolución, desde S/ 0.8 por día. Cómpralo haciendo clic aquí: ',
      today: '',
      client: {
  ...
  mounted () {
    ...
    let goPath = (this.$store.state.common.channel === 'SEGURO_UNIVERSITARIO') ? '/seguro-de-vida/universitario' : (this.$store.state.common.channel === 'AHORRO_JUBILACION') ? '/seguro-de-vida/ahorro-jubilacion' : (this.$store.state.common.channel === 'PROTECCION_SEGURA') ? '/seguro-de-vida/proteccion-segura' : (this.$store.state.common.channel === 'AHORRO_SEGURO') ? '/seguro-de-vida/ahorro-seguro' : '/seguro-de-vida'
    this.currentChannel = (this.$store.state.common.channel === 'IBK') ? 'Vida Devolución' : (this.$store.state.common.channel === 'SEGURO_UNIVERSITARIO') ? 'Seguro Universitario' : (this.$store.state.common.channel === 'AHORRO_JUBILACION') ? 'Ahorro Jubilación' : (this.$store.state.common.channel === 'PROTECCION_SEGURA') ? 'Protección Segura' : (this.$store.state.common.channel === 'AHORRO_SEGURO') ? 'Ahorro Seguro' : 'Vida con Devolución'
    this.urlShared = window.location.origin + goPath
    /* eslint-disable */
    ...
    if (this.$store.state.payment.numeroPoliza) {
      // Enviar poliza si sus datos NO estan hasheados
      if (!this.$store.state.common.hashPhone && !this.$store.state.common.hashEmail) {
        this.sendPoliza(this.$store.state.common.saveClient.email, this.$store.state.payment.numeroPoliza)
      }
      window.dataLayer.push({
        'event': 'virtualPageView',
        'pageTitle': 'Confirmar | ' + process.env.channel[process.env.subChannel[this.$store.state.common.subchannel]].PAGE_TITLE,
        'pageUrl': process.env.channel[process.env.subChannel[this.$store.state.common.subchannel]].URL_BASE + 'paso/confirmar/'
      })
      window.dataLayer.push({
        'event': 'Confirmar venta',
        'category': 'Ecommerce',
        'action': 'Purchase',
        'label': 'venta realizada',
        'venta_id': this.$store.state.payment.numeroPoliza,
        'porcentaje': this.$store.state.common.cotizacion.porcentaje,
        'sumaAsegurada': this.$store.state.common.cotizacion.sumaAsegurada,
        'periodo vigencia': this.$store.state.common.cotizacion.periodoVigencia,
        'frecuencia pago': this.$store.state.common.cotizacion.frecuenciaPago
      })
      // Amplitude
      let frecuenciaPagoAmplitude = (this.$store.state.common.cotizacion.frecuenciaPago === 'M') ? 'Mensual' : (this.$store.state.common.cotizacion.frecuenciaPago === 'T') ? 'Trimestral' : (this.$store.state.common.cotizacion.frecuenciaPago === 'S') ? 'Semestral' : (this.$store.state.common.cotizacion.frecuenciaPago === 'A') ? 'Anual' : this.$store.state.common.cotizacion.frecuenciaPago
      this.$amplitude.getInstance().identify(new this.$amplitude.Identify().add('Cantidad de polizas vida comprada', 1))
      this.$amplitude.getInstance()
      .logEvent(
        'Vida - Compra Finalizada',
        {
          'Producto Consultado': 'VIDA',
          'Sub Producto Consultado': this.$store.state.common.channel,
          'Asesoria': (this.$store.state.common.agente) ? true : false,
          'Nombre de Asesor': (this.$store.state.common.agente) ? this.$store.state.common.agentes[this.$store.state.common.agente].nombre : '',
          'Canal de Venta': 'Online',
          'Número de Poliza': this.$store.state.payment.numeroPoliza,
          'Monto a Pagar': this.$store.state.common.cotizacion.primaRecurrente,
          'Cantidad Asegurada': this.$store.state.common.cotizacion.sumaAsegurada,
          'Periodo': this.$store.state.common.cotizacion.periodoPago,
          'Frecuencia de Pago': frecuenciaPagoAmplitude,
          '% de Devolucion': this.$store.state.common.cotizacion.porcentaje,
          'Estado Usuario': 'Usuario No Logueado'
        }
      )
      let frecuenciaPagoText = (this.$store.state.common.cotizacion.frecuenciaPago === 'M') ? 'mensual' : (this.$store.state.common.cotizacion.frecuenciaPago === 'T') ? 'trimestral' : (this.$store.state.common.cotizacion.frecuenciaPago === 'S') ? 'semestral' : 'anual'
      ...
      window.dataLayer.push({
        ...
        'ecommerce': {
          'purchase': {
            'actionField': {
              'id': this.$store.state.payment.numeroPoliza,
              'revenue': (primaFrecuencia).toString(),
            },
            'products': [{ 
              'name': productName,   
              'id': productId,
              ...
              'quantity': '1',
              'category': 'Póliza',
              'metric1': this.$store.state.common.cotizacion.sumaAsegurada,
              'dimension2': this.$store.state.common.cotizacion.porcentaje,
              'dimension3': this.$store.state.common.cotizacion.periodoVigencia,        
              'dimension4': frecuenciaPagoText
            }]
          }
        }
      });
  ...
  methods: {
    copyURL (e, type) {
      window.dataLayer.push({
        'event': 'Confirmar compartir Copiar',
        'category': 'UI :: Compra realizada',
        'action': 'Click',
        'label': 'compartir por copia'
      })
      e.preventDefault()
      if (type === 'mobile') {
      ...
      setTimeout(function () {
        let goPath = (this.$store.state.common.channel === 'IBK') ? '/interbank' : (this.$store.state.common.channel === 'SEGURO_UNIVERSITARIO') ? '/universitario' : (this.$store.state.common.channel === 'AHORRO_JUBILACION') ? '/ahorro-jubilacion' : (this.$store.state.common.channel === 'PROTECCION_SEGURA') ? '/proteccion-segura' : (this.$store.state.common.channel === 'AHORRO_SEGURO') ? '/ahorro-seguro' : '/vida-con-devolucion'
        this.urlShared = window.location.origin + goPath
        var Url = document.getElementById(type + '_url')
        Url.value = this.urlShared + '/?utm_source=Clipboard&utm_campaign=share&utm_medium=share'
    ...
    updateDep () {
      let self = this
      if (self.formPlaft.departamento !== '0') {
        self.provinciaFilter = self.provincia.filter(p => p.department_id == self.formPlaft.departamento)
        self.formPlaft.provincia = self.provinciaFilter[0].id
        self.updateProv()
      }
    },
    updateProv () {
      let self = this
      if (this.formPlaft.provincia !== '0') {
        this.formPlaft.provincia
        this.distritoFilter = this.distrito.filter(d => d.department_id == self.formPlaft.departamento && d.province_id == self.formPlaft.provincia)
        self.formPlaft.distrito = this.distritoFilter[0].id
      }
    },
    goSteps (step) {
      let self = this
      if (step === '3') {
        let formularioPlaft =  self.formPlaft
        formularioPlaft.profesion = self.profesion.filter(d => d.codigo == self.formPlaft.profesion)[0].nombre
        formularioPlaft.departamento = self.departamento.filter(d => d.id == self.formPlaft.departamento)[0].name
        formularioPlaft.provincia = self.provincia.filter(p => p.id == self.formPlaft.provincia)[0].name
        formularioPlaft.distrito = self.distrito.filter(d => d.id == self.formPlaft.distrito)[0].name
        this.$store.dispatch('payment/saveClientPlaft', { 
            'numeroDocumento': self.$store.state.common.dniCliente, 
            'sortedKeys': [
              'profesion', 'departamento', 'provincia', 'distrito', 'direccion'
            ],
            'campos': self.formPlaft
          })
        .then(resSend => {
          if (resSend.code === '01') {
            this.stepClient = step
          } else {
            Swal.fire(
              'Oops...!',
              'Estamos teniendo problemas para guardar sus datos. Para mayor información puedes comunicarte al +51 933559693',
              'error'
            )
            this.tracking.errorType = 'FRONTEND'
            this.tracking.errorData = 'Estamos teniendo problemas para guardar sus datos. Para mayor información puedes comunicarte al +51 933559693'
            this.$store.dispatch('common/makeTracking', this.tracking).then(resTracking => {
    ...
    socialEventTwitter () {
      window.dataLayer.push({
        'event': 'Confirmar compartir Twitter',
        'category': 'UI :: Compra realizada',
        'action': 'Click',
    ...
    socialEventFacebook () {
      window.dataLayer.push({
        'event': 'Confirmar compartir Facebook',
        'category': 'UI :: Compra realizada',
        'action': 'Click',
    ...
    socialEventWhatsapp () {
      window.dataLayer.push({
        'event': 'Confirmar compartir Whatsapp',
        'category': 'UI :: Compra realizada',
        'action': 'Click',
    ...
    goZonaSegura () {
      ...
      //- window.location.replace("https://www.interseguro.pe/zona-segura/")
    },
    sendPoliza (correo, numeroPoliza) {
      // this.$store.dispatch('payment/sendPolizaWithout', { 'email': correo, 'numeroPoliza': numeroPoliza })
      this.$store.dispatch('payment/sendPoliza', { 'email': correo, 'numeroPoliza': numeroPoliza })
      .then(resSend => {
        if (resSend.code === '01') {
          ...
          Swal.fire(
            'Oops...!',
            'Estamos teniendo problemas para enviar su póliza. Para mayor información puedes comunicarte al +51 933559693',
            'error'
          )
          this.tracking.errorType = 'FRONTEND'
          this.tracking.errorData = 'Estamos teniendo problemas para enviar su póliza. Para mayor información puedes comunicarte al +51 933559693'
          this.$store.dispatch('common/makeTracking', this.tracking).then(resTracking => {
            // console.log('Tracking pago', resTracking)
          })
          this.$amplitude.getInstance().logEvent(
    ...
    updateSendPoliza () {
      ...
      // API enviar poliza
      this.$v.$touch(false)
      if (!this.$v.client.$error && this.client.correo && this.client.numeroTelefono) {
        ...
        this.$store.dispatch('payment/updatePoliza', { 'asegurado': client, 'beneficiarios':  null, 'numeroPoliza': this.$store.state.payment.numeroPoliza })
        // this.$store.dispatch('payment/updatePolizaWithout', { 'asegurado': client, 'beneficiarios':  null, 'numeroPoliza': this.$store.state.payment.numeroPoliza })
        .then(resUpdate => {
          if (resUpdate.code === '01') {
            this.sendPoliza(this.client.correo, this.$store.state.payment.numeroPoliza)
          } else {
            Swal.fire(
              'Oops...!',
              'Estamos teniendo problemas para actualizar su póliza. Para mayor información puedes comunicarte al +51 933559693',
              'error'
            )
            this.tracking.errorType = 'FRONTEND'
            this.tracking.errorData = 'Estamos teniendo problemas para actualizar su póliza. Para mayor información puedes comunicarte al +51 933559693'
            this.$store.dispatch('common/makeTracking', this.tracking).then(resTracking => {
              // console.log('Tracking pago', resTracking)
            })
            this.$amplitude.getInstance().logEvent(
        ...
        this.tracking.errorType = 'FRONTEND'
        this.tracking.errorData = 'Verifique sus datos'
        this.$store.dispatch('common/makeTracking', this.tracking).then(resTracking => {
          // console.log('Tracking pago', resTracking)
        })
        this.$amplitude.getInstance().logEvent(
  ...
  beforeDestroy () {
    // CORREO HASHEADO
    if (this.$store.state.common.hashEmail) {
      this.sendPoliza(this.$store.state.common.saveClient.email, this.$store.state.payment.numeroPoliza)
    }
    if (localStorage.removeItem('jwt') !== null) {
      this.$store.commit('common/setCotizacion', [])
      this.$store.commit('common/setDniCliente', '')
      this.$store.commit('common/setSaveClient', { numeroDocumento: '', nombre: '', apellidoPaterno: '', apellidoMaterno: '', fechaNacimiento: '', sexo: '', numeroTelefono: '', email: '', numeroDocumento: '' })
    }
  },
  destroyed () {
    this.$store.commit('common/setCotizacion', [])
